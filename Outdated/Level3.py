...
from Actors.Enemies import EnenemyJet
...
class Level3:
    ...
    def run(self):
        ...
        while running:
            ...
            # ----------------------------- UPDATE HUD AND DRAW IT ------------------------------------------------
            hud.update(currentWave, score, maxWaves, self.player.health, self.player.maxHealth, self.player.lives, self.player.damage,
                       600 - sBooster, self.player.bspeed)
            self.manager.draw_ui(self.screen)
